...
# Lista/Comando de búsqueda (choco search)

Chocolatey realizará una búsqueda de un paquete local o remoto. Algunos pueden preferir utilizar el comando 
[[`clist`|Commandslist]] como un acceso directo de [[`choco list`|Commandslist]].

**NOTA:** 100% compatible con el cliente Chocolatey anterior (0.9.8.x e inferior)
 con opciones e interruptores. IEn la mayoría de los casos, aún puede pasar opciones e interruptores con un guión (`-`). 
...
**NOTA:** Vea en scripts [cómo pasar argumentos |CommandsReference#how-to-pass-options--switches]] (`choco -?`) 
para conocer cómo escribir scripts e integraciones adecuados.
...
choco proporcionará códigos de salida mejorados que permiten una mejor integración y secuencias de comandos.

Si encuentra otros códigos de salida que aún no hemos documentado, presente un ticket para que podamos documentarlo en
...
### WebPI
Especifica que la fuente es Web PI (Web Platform Installer) y que estamos buscando un producto WebPI, como IISExpress. 
Si no tiene instalada la línea de comandos de Web PI, se instalará primero y luego se realizará la búsqueda solicitada
por ejemplo. `choco list --source webpi`

### Características de Windows
Esto especifica que la fuente es una característica de Windows y deberíamos instalarla a través de la herramienta de administración 
y mantenimiento de imágenes de implementación (DISM) en la máquina local.
por ejemplo. `choco list --source windowsfeatures`

## Opciones e interruptores

**NOTA:** Options and switches apply to all items passed, so if you are
