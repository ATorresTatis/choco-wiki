...
# Comando generador de paquetes (choco new)

Las [ediciones con licencia de Chocolatey](https://chocolatey.org/compare) incluyen algún nivel de Package Builder.

 - Las ediciones Professional/MSP (ediciones comerciales superiores a la versión 1.9.0) 
   pueden aprovechar la interfaz de usuario de Package Builder excepto la detección automática.
 - Business/Architect (en ediciones comerciales superiores a 1.4.0) pueden usar –file o 
   la interfaz de usuario de Package Builder para generar una implementación completamente 
   desatendida en tan solo segundos. Es tan fácil que un niño de 7 años puede hacerlo. 
   Haga clic derecho en 'Crear paquete ...'
 - Business/Architect (en ediciones comerciales superiores a 1.8.0) pueden usar 
   `--from-programs-and-features` para generar paquetes directamente desde 
   Programas y Características del sistema operativo.

Vea https://chocolatey.org/docs/features-create-packages-from-installers

# Comando Nuevo (choco new)

Chocolatey generará archivos de especificación de paquete para un nuevo paquete
...
Posibles propiedades que se pueden utilizar:
    packageversion
    maintainername
    maintainerrepo
    installertype
...
**NOTA:** a partir de la versión 0.9.10, puede pasar pares de valores de propiedad arbitrarios 
  a las plantillas. Esto realmente desbloquea la capacidad de crear paquetes automáticamente.

**NOTA:** [Chocolatey for Business](https://chocolatey.org/compare) puede crear paquetes 
  completos con solo apuntar el comando nuevo a instaladores nativos.

**NOTA:** [Chocolatey for Business](https://chocolatey.org/compare) puede descargar e 
...
por lo que, si está ejecutando un comando como por ejemplo la instalación que permite 
desplegar varios paquetes, y utiliza el parámetro `--version=1.0.0`, se buscará e intentará 
instalar la versión 1.0.0 de cada paquete aprobado. Por lo tanto, divida varias llamadas 
de paquete cuando desee utilizar opciones específicas.

Incluye [[opciones/interruptores predeterminados|CommandsReference#default-options-and-switches]] (se incluyen a continuación para completar).
...
***NOTA:*** Esta documentación ha sido generada automáticamente desde `choco new -h`.
